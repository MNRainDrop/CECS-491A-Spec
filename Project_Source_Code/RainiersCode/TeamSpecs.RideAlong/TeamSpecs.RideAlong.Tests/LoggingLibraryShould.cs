...
public class LoggingLibraryShould
{
    ...
    [Fact]
    public void LL_Log_RetrieveLogsWithSetValue_LogsWillBeRetrievedFromDataStore_Pass()
    {
        // Arrange
        var timer = new Stopwatch();
        IResponse response;
        var logService = new LogService(new SqlDbLogTarget(new SqlServerDAO()));
        var logFilter = new LogFilter(logLevel: "Info");

        // Expected values
        var expectedHasError = false;
        string? expectedErrorMessage = null;
        var expectedReturnValue = "Info";

        // Act
        timer.Start();
        response = logService.RetrieveLog(logFilter);
        timer.Stop();
        ...
        if (response.ReturnValue is not null)
        {
            ...
            {
                Assert.Fail("response.ReturnValue.Count should return values");
            }
            foreach (ILog log in response.ReturnValue)
            {
                Assert.True(log.LogLevel == expectedReturnValue);
            }
        }
        else
        {
    ...
    [Fact]
    public void LL_Log_RetrieveLogsWithInvalidSetValue_NoLogsWillBeRetrievedFromDataStore_Pass()
    {
        // Arrange
        var timer = new Stopwatch();
        IResponse response;
        var logService = new LogService(new SqlDbLogTarget(new SqlServerDAO()));
        var logFilter = new LogFilter(logLevel: "This will not work");
        ...
        if (response.ReturnValue is not null)
        {
            Assert.True(response.ReturnValue.Count == expectedReturnCount);
        }
        else
        {
            Assert.Fail("response.ReturnValue should not be null");
    ...
    [Fact]
    public void LL_Log_RetrieveLogsWithNoSetValue_AllLogsWillBeRetrievedFromDataStore_Pass()
    {
        // Arrange
        var timer = new Stopwatch();
        IResponse response;
